    ...
    return (
        <div className='container mx-auto p-4'>
            ...
            {all_product.map((e) => {
                if (cartItems[e.id] > 0) {
                    return (
                        <div key={e.id} className='item-format grid grid-cols-6 gap-4 text-center items-center border-b py-2'>
                            <img src={e.image} alt={e.name} className='h-16 w-16 object-cover mx-auto' />
                            <p className='text-sm'>{e.name}</p>
                            <p>${e.new_price}</p>
                            <button className='bg-gray-200 h-12 w-12 mx-auto rounded'>{cartItems[e.id]}</button>
              ...
              <div className='cart-items flex flex-col md:flex-row justify-between mt-8'>
                <div className='cart-items-total w-full md:w-2/3'>
                    ...
                    <button className='px-4 py-2 bg-red-500 text-white hover:bg-red-600 mt-8 mx-16'>PROCEED TO CHECKOUT</button>
                </div>
                <div className='promo-code w-full md:w-1/3 p-4 mt-24'>
                    <p>If you have a promo code, enter it here:</p>
                    <div className='flex mt-4'>
                        <input type='text' placeholder='Promo code' className='border p-2 flex-grow bg-gray-200' />
